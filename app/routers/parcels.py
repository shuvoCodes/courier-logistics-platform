...
def get_all_parcels(user: user_dependancy, db: db_dependancy):
    ...
    if user.role == 'admin':
        ...
        return query.filter(Parcel.sender_id == user.id).all()
...
def list_parcels(db: db_dependancy,user: user_dependancy,search: str = Query(None),status: str = Query(None),category: str = Query(None),
    ...
    if user.role != "admin":
        query = query.filter(Parcel.sender_id == user.id)
...
def delete_parcel(db: db_dependancy, user:user_dependancy,parcel_id: int):
    ...
    db.delete(parcel)
    db.commit()
    return JSONResponse(status_code=200, content={ "message": "Parcel deleted successfully"})from datetime import datetime
from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy.orm import Session
